...
node evaluator FAIL supplies a failure observation only for deterministic criteria,
verification, patch preflight, or model protocol failures. Runtime capacity,
transport and control failures do not train model quality even if the scheduler
also persisted a node evaluator FAIL. Passing nodes in a graph whose
...
across graph revisions are excluded. Within the same revision, a resumed PASS
requires both the current retained record and its original execution, with every
non-lifecycle field unchanged. Original evidence and any required Task Review are
revalidated; timing comes from the original execution, excluding paused wait.
Replay reads facts and never trains a model,
