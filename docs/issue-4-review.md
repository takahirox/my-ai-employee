...
| `GraphRunRecord`, `NodeRouteRecord`, `NodeExecutionRecord`, and Inspector | Top-level assessment/strategies are already durable. Each Graph-owned node route stores its assessment, eligible IDs, selected strategy, effective-policy digest, and Harness digest; execution records link back by route digest and carry terminal status, evaluator decision, retries/attempt, and failure evidence. A second telemetry authority is not needed. |
