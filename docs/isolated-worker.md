...
An explicitly supplied **scoped** auth file is copied into the disposable worker's
...
With scoped authentication, an internal Docker network reaches a dedicated CONNECT
...
This proves the plumbing, **not a model productivity improvement**. The required
same-model proposal-vs-isolated comparison remains outstanding. Run both against the
...
Do not close #81 until that comparison and the remaining budget/security review are
complete. Never redeem a reset ticket to finish the comparison.
