...
ROLE_DEFAULTS: Mapping[ContextRole, ContextPolicy] = {
    ContextRole.PLANNER: ContextPolicy(
        id="context-planner", role=ContextRole.PLANNER, max_items=50, max_bytes=64_000,
        include_history=False,
    ),
    ContextRole.WORKER: ContextPolicy(
        id="context-worker", role=ContextRole.WORKER, max_items=20, max_bytes=96_000,
        include_history=False,
    ),
    ContextRole.VERIFIER: ContextPolicy(
        id="context-verifier", role=ContextRole.VERIFIER, max_items=30, max_bytes=96_000,
        include_history=False, allowed_reference_kinds=("document", "artifact", "evidence"),
    ),
    ContextRole.REVIEWER: ContextPolicy(
        id="context-reviewer", role=ContextRole.REVIEWER, max_items=40, max_bytes=128_000,
        include_history=False, allowed_reference_kinds=("artifact", "evidence", "document"),
    ),
}
...
class ContextCompiler:
    ...
    def compile(
        ...
        *,
        ...
        eligible = tuple(
            sorted((item for item in references if item.kind in allowed), key=lambda item: (item.kind, item.target_id))
        )
        ...
        for reference in eligible:
            ...
            if len(selected) >= selected_policy.max_items or used_bytes + size > selected_policy.max_bytes:
        ...
        digest_input = {
            "run_id": run_id, "role": role.value, "policy": selected_policy,
            "references": selected, "sources": {item.target_id: sources.get(item.target_id) for item in selected},
        }
        return ContextPackage(
            id=package_id, run_id=run_id, role=role, policy_id=selected_policy.id,
            compiled_at=datetime.now(timezone.utc), authoritative_refs=tuple(selected),
            inline_items=inline, omitted_refs=tuple(omitted), source_digest=canonical_digest(digest_input),
        )
