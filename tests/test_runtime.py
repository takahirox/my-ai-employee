...
class RuntimeTests(unittest.TestCase):
    def test_end_to_end_demo_repairs_persists_completes_and_replays(self) -> None:
        with SQLiteStore(":memory:") as store:
            ...
            self.assertEqual(projection["graph"]["entry_task_ids"], ["prepare"])
            self.assertEqual(projection["graph"]["terminal_task_ids"], ["complete"])
            self.assertEqual(len(projection["graph"]["edges"]), 5)
